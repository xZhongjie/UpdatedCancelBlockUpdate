...
> **注意**：1.21.11 起 Minecraft 重构了游戏规则系统（`GameRules` 移到 `world.rule` 包，`Key/Type/Rule` 改为 `GameRule/GameRuleType`），单独一个分支；`1.21.4`/`1.21.8` 分支的 jar **不适用于 1.21.11 及以上**（范围已通过 `depends.minecraft` 收紧，加载时会直接拒绝，不会崩溃）。26.x 起 Minecraft 改为 Mojang 官方命名且要求 Java 25，26.1/26.2 分支独立维护。
